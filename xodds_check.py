...
print("="*112)
print("  2. CROSS-MATCH ODDS - IS THE COMPARISON EVEN VALID?")
print("     prev_odds is her last quoted Over price on a PREVIOUS night. If her LINE moved")
print("     between games, that price belongs to a DIFFERENT number and comparing them is")
print("     meaningless - the same bug class as the close-price one.")
print("="*112)
X = [r for r in S if r["xodds"] is not None and r["prev_ln"] is not None]
same = [r for r in X if r["prev_ln"] == r["first_ln"]]
diff = [r for r in X if r["prev_ln"] != r["first_ln"]]
print(f"  starred bets with a previous-game price   {len(X)}")
print(f"    previous quote at the SAME line          {len(same)}")
print(f"    previous quote at a DIFFERENT line       {len(diff)}   <- comparison invalid here")
print("")
print("  the 'market drifted away from her' finding, split by whether the comparison is valid:")
st([r for r in X    if r["xodds"] >= 0], "  ALL drifted        (what I nearly reported)")
st([r for r in same if r["xodds"] >= 0], "  CLEAN drifted      (same line both games)", minn=10)
st([r for r in diff if r["xodds"] >= 0], "  INVALID drifted    (line moved between games)", minn=10)
print("")
st([r for r in X    if r["xodds"] < 0],  "  ALL shortened", minn=10)
st([r for r in same if r["xodds"] < 0],  "  CLEAN shortened", minn=10)
print("")
print("="*112)
print("  AND OUT OF SAMPLE, on the clean subset only")
print("="*112)
dates = sorted({r["date"] for r in B}); cut = dates[int(len(dates)*0.6)]
CL = [r for r in same]
for lbl, fn in (("clean, drifted", lambda r: r["xodds"] >= 0), ("clean, shortened", lambda r: r["xodds"] < 0)):
    st([r for r in CL if fn(r) and r["date"] <  cut], f"  {lbl}  IN", minn=8)
    st([r for r in CL if fn(r) and r["date"] >= cut], f"  {lbl}  OUT", minn=8)
    print("")
