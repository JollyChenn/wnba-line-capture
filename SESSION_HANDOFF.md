...
- **GitHub free-tier is the #1 ops risk** — VERIFIED 2026-06-20 via `gh run list`: it delays grade runs ~1.5-2h (15:23→~17:30, 18:23→~20:00) AND silently DROPS early-AM crons (the 05:23/06:23 added that morning never fired). This stranded the 6/19 slate un-graded ~13h. Grading logic itself is correct (audited 39/39 via `audit_results.py`). **TWO independent fixes now in place:** (1) cloud dense sweep 04:41–09:41 (6 idempotent grade crons); (2) **laptop Task-Scheduler task `WNBA-Grade-Trigger`** (`grade_trigger.bat` → `gh workflow run grade-bets.yml`, 13/16/19/22 SG-local = right when the SG-time user's overnight-US games settle; GitHub ALWAYS honors dispatch). Tested green end-to-end. Remove laptop side: `schtasks /delete /tn "WNBA-Grade-Trigger" /f`. Laptop-OFF-all-day gap remains → external web-cron (cron-job.org→dispatch API) is the only 100% fix.
