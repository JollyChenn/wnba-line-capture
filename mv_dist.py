...
d = C.Counter(round(r["first_ln"] - r["prev_ln"], 1) for r in K)
print("EXACT DISTRIBUTION of (tonight's line - her previous game's line)")
print(f"  {len(K)} candidates with a previous line")
print("")
for mv in sorted(d):
    bar = "#" * min(d[mv], 40)
    flag = ""
    if mv >= 0.5: flag = "  <- REJECTED by the star"
    print(f"   {mv:+5.1f}  {d[mv]:>4}  {bar}{flag}")
print("")
half = sum(v for k, v in d.items() if abs(k*2 - round(k*2)) > 1e-6)
print(f"  moves that are NOT a multiple of 0.5: {half}")
print(f"  moves of exactly +0.5: {d.get(0.5,0)}     moves of exactly -0.5: {d.get(-0.5,0)}")
print("")
print("  This is what makes the star a real threshold rather than an arbitrary one: the book")
print("  does not nudge these numbers by half a point. It holds them, or it moves them by 1+.")
