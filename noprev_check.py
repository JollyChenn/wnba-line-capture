...
print("="*110)
print("  THE THIRD GROUP NOBODY NAMED: bets with NO PREVIOUS LINE AT ALL")
print("  (her first game on the board, or the market was not quoted last time)")
print("="*110)
noprev = [r for r in K if r["prev_ln"] is None]
haveprev = [r for r in K if r["prev_ln"] is not None]
print(f"  candidates total          {len(K)}")
print(f"    have a previous line    {len(haveprev)}")
print(f"    NO previous line        {len(noprev)}   <- what do we do with these?")
print("")
st(noprev,   "  NO previous line")
st([r for r in haveprev if r["first_ln"]-r["prev_ln"] <  0.5], "  cut or held (true star)")
st([r for r in haveprev if r["first_ln"]-r["prev_ln"] >= 0.5], "  raised")
print("")
print("="*110)
print("  THE DIVERGENCE - the live card and my analysis disagree about the no-prev group")
print("="*110)
print("  model_card.py:  raised = (pv is not None and line - pv >= 0.5)")
print("                  -> pv is None gives raised=False -> the card BETS them as starred")
print("  my analysis:    notraised = (pv is not None and line - pv < 0.5)")
print("                  -> pv is None gives notraised=False -> the analysis SKIPS them as raised")
print("  Both cannot be right. Scoring the two readings:")
print("")
st([r for r in K if r["prev_ln"] is None or r["first_ln"]-r["prev_ln"] < 0.5],
   "  CARD's rule    (no-prev counted as a BET)")
st([r for r in haveprev if r["first_ln"]-r["prev_ln"] < 0.5],
   "  ANALYSIS rule  (no-prev EXCLUDED)")
