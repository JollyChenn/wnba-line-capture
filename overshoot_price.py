...
def line(rows, label, key):
    n = len(rows)
    if n < 15:
        print(f"    {label:<34} {key:<6} n={n} too few"); return
    w = sum(1 for r in rows if r["won"])
    u = sum((r[key]-1) if r["won"] else -1.0 for r in rows)
    mo = sum(r[key] for r in rows)/n
    print(f"    {label:<34} {key:<6} n={n:<4} {100*w/n:5.1f}%  mean odds {mo:.3f}"
          f"  {u:+7.2f}u  ROI {100*u/n:+6.1f}%")

S = [r for r in B if r["notraised"]]
GROUPS = [("flip + hotover  <- LIVE", ("flip","hotover")),
          ("overshoot", ("overshoot",)),
          ("flip + hotover + overshoot", ("flip","hotover","overshoot"))]

print("="*100)
print("  1. SAME BETS, THREE REAL PRICES.  first = what the alert showed.")
print("     last = last price we logged before tip.  close = board final quote.")
print("="*100)
for lbl, keys in GROUPS:
    rows = [r for r in S if r["src"] in keys]
    have = [r for r in rows if r["close"]]
    print(f"  {lbl}   ({len(have)} of {len(rows)} have all three prices)")
    for k in ("first","last","close"):
        line(have, lbl, k)
    print("")

print("="*100)
print("  2. THE ODDS THEMSELVES - is overshoot bet at worse prices than flip?")
print("="*100)
for lbl, keys in GROUPS:
    rows = [r for r in S if r["src"] in keys]
    od = sorted(r["first"] for r in rows)
    n = len(od)
    if not n: continue
    print(f"  {lbl:<34} n={n:<4} min {od[0]:.2f}  p25 {od[n//4]:.2f}  median {od[n//2]:.2f}"
          f"  p75 {od[3*n//4]:.2f}  max {od[-1]:.2f}")
print("")
print("="*100)
print("  3. STRESS TEST - what if you always got a WORSE price than logged?")
print("="*100)
for lbl, keys in GROUPS:
    rows = [r for r in S if r["src"] in keys]
    if len(rows) < 15: continue
    w = sum(1 for r in rows if r["won"]); n = len(rows)
    print(f"  {lbl:<34} actually hit {100*w/n:.1f}%  (n={n})")
    for px in (1.90, 1.85, 1.80, 1.75, 1.70):
        u = w*(px-1) - (n-w)
        flag = "profitable" if u > 0 else "LOSES"
        print(f"      every bet flat at {px:.2f}   break-even {100/px:.1f}%   -> {u:+7.2f}u  {flag}")
    print("")
