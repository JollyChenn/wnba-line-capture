...
K = [r for r in B if r["src"] in ("flip","hotover","overshoot") and r["prev_ln"] is not None]
for r in K: r["star"] = (r["first_ln"] - r["prev_ln"]) < 0.5
dates = sorted({r["date"] for r in B}); cut = dates[int(len(dates)*0.6)]
DAYS = len(dates)
def st(rows, label, minn=12):
    n = len(rows)
    if n < minn:
        print(f"  {label:<40} n={n:<4} too few"); return None
    ...
    print(f"  {label:<40} n={n:<4} {100*w/n:5.1f}%  {u:+7.2f}u  ROI {100*u/n:+6.1f}%"
          f"  alpha {100*(w/n-b):+5.1f}pp  z={z:+5.2f}  {n/DAYS:.2f}/night")
    return u/n
print("="*112)
print("  THE MARKETS WE EXCLUDED - were they judged RAW, before the star existed?")
print("  (this is exactly how overshoot was nearly lost: ranked raw, cut, never re-tested)")
print("="*112)
for mk in ("pra","pr","pts","pa","reb","ast","ra"):
    inlive = "  <- IN" if mk in ("pra","pr","pts") else ""
    print(f"  --- {mk} ---{inlive}")
    st([r for r in K if r["mk"] == mk],                  f"    {mk} RAW")
    st([r for r in K if r["mk"] == mk and r["star"]],    f"    {mk} STARRED")
    st([r for r in K if r["mk"] == mk and not r["star"]],f"    {mk} raised")
    print("")
print("="*112)
print("  WHAT ADDING EACH EXCLUDED MARKET WOULD DO (starred only)")
print("="*112)
LIVE = [r for r in K if r["mk"] in ("pra","pr","pts") and r["star"]]
st(LIVE, "MODEL S as-is")
for mk in ("pa","reb","ast","ra"):
    st([r for r in K if r["mk"] in ("pra","pr","pts",mk) and r["star"]], f"  + {mk}")
st([r for r in K if r["star"]], "  + ALL markets")
print("")
print("  and out of sample for any that look worth it:")
for mk in ("pa","reb","ast","ra"):
    g = [r for r in K if r["mk"] == mk and r["star"]]
    if len(g) < 12: continue
    st([r for r in g if r["date"] <  cut], f"  {mk} starred IN")
    st([r for r in g if r["date"] >= cut], f"  {mk} starred OUT")
