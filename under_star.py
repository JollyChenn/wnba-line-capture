...
print("="*104)
print("  THE FUNNEL - where the volume actually goes")
print("="*104)
allov = build2("Over", ("flip","hotover","overshoot"))
allun = build2("Under", ("newunder","model","starout","fragile"))
pap  = build2("Over", ("flip_paper","cascade"))
print(f"  deduped to one bet per player-market-GAME:")
print(f"    our live signals (flip/hotover/overshoot) OVERS   {len(allov)}")
print(f"    dead signals (flip_paper/cascade) OVERS           {len(pap)}")
print(f"    every UNDER signal                                {len(allun)}")
print("")
print("  so the raw 5202 newunder rows in bets_log are the SAME bets re-logged every capture.")
print("  deduped, the under stream is only ~1.6x our over stream - not 10x.")
print("")
ov3 = [r for r in allov if r["mk"] in ("pra","pr","pts")]
ovs = [r for r in ov3 if r["mv"] is not None and r["mv"] < 0.5]
print(f"  MODEL S funnel: {len(allov)} over signals -> {len(ov3)} in pra/pr/pts -> {len(ovs)} starred")
print("")
print("="*104)
print("  THE UNTESTED IDEA: the STAR, mirrored for unders (book did NOT CUT her number)")
print("="*104)
for lbl, rows, cut_is_bad in (("OVERS  (star = not RAISED)", ov3, False),
                              ("UNDERS (star = not CUT)",    [r for r in allun if r["mk"] in ("pra","pr","pts")], True)):
    print(f"  --- {lbl} ---")
    show2(rows, "  everything")
    have = [r for r in rows if r["mv"] is not None]
    if cut_is_bad:
        show2([r for r in have if r["mv"] > -0.5], "  STARRED: line held or ROSE")
        show2([r for r in have if r["mv"] <= -0.5], "  book CUT her number (already repriced)")
    else:
        show2([r for r in have if r["mv"] < 0.5],  "  STARRED: line held or was cut")
        show2([r for r in have if r["mv"] >= 0.5], "  book RAISED her number")
    show2([r for r in rows if r["mv"] is None],    "  no previous line")
    print("")
