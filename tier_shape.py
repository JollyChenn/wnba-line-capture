...
print("="*100)
print("  IS THE TIER PATTERN MONOTONIC, OR IS 'THIN AND STRONG BUT NOT SOLID' A NOISE SHAPE?")
print("="*100)
for t in ("THIN","SOLID","STRONG"):
    st([r for r in K if r["tier"] == t], f"  {t}")
print("")
print("  a non-monotonic ordering (good, bad, good) is the classic shape of a noise split.")
print("  monotonic would be THIN < SOLID < STRONG or the reverse.")
print("")
print("="*100)
print("  DOES THE STAR HELP INSIDE flip_paper THIN/STRONG?")
print("="*100)
TS = [r for r in K if r["tier"] in ("THIN","STRONG")]
st(TS, "  THIN/STRONG, all")
st([r for r in TS if r["star"]],     "  THIN/STRONG + starred")
st([r for r in TS if not r["star"]], "  THIN/STRONG, not starred")
print("")
print("  if the star does nothing here, adding this source means running TWO different rules")
print("  (star-gated for flip/hotover/overshoot, tier-gated for flip_paper) - more surface area")
print("  for a fitted result to hide in.")
print("")
print("="*100)
print("  MONTH BY MONTH")
print("="*100)
for mo in sorted({r["mo"] for r in TS}):
    st([r for r in TS if r["mo"] == mo], f"  THIN/STRONG {mo}", minn=8)
