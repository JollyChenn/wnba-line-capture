...
for r in K: r["mv"] = r["first_ln"] - r["prev_ln"]
...
print("="*106)
print("  SHOULD WE LOOSEN THE STAR TO ALLOW A 1-POINT RAISE?")
print("="*106)
STAR  = [r for r in K if r["mv"] < 0.5]
R1    = [r for r in K if 0.5 <= r["mv"] < 2.0]
R2    = [r for r in K if r["mv"] >= 2.0]
st(STAR,        "MODEL S as-is (cut or held)          <- LIVE")
st(R1,          "  the band in question: raised 1.0-2.0")
st(R2,          "  raised 2.0+ (stays out either way)")
st(STAR + R1,   "LOOSENED: star + raised-up-to-2.0")
print("")
print("  out of sample, split " + cut + ":")
for lbl, g in (("MODEL S as-is", STAR), ("raised 1.0-2.0 alone", R1), ("LOOSENED", STAR + R1)):
    st([r for r in g if r["date"] <  cut], f"  {lbl}  IN")
    st([r for r in g if r["date"] >= cut], f"  {lbl}  OUT")
    print("")
print("  month by month for the band in question:")
for mo in sorted({r["mo"] for r in R1}):
    st([r for r in R1 if r["mo"] == mo], f"    raised 1.0-2.0  {mo}", minn=5)
