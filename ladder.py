...
def st(rows, label, minn=12):
    ...
    print(f"  {label:<48} n={n:<4} {100*w/n:5.1f}%  {u:+8.2f}u  ROI {100*u/n:+6.1f}%"
          f"  alpha {100*(w/n-b):+5.1f}pp  z={z:+5.2f}")

print("="*112)
print("  THE LADDER: start from every over the engine has ever fired, add one restriction at a")
print("  time. This is what each filter is actually worth.")
print("="*112)
L1 = B
L2 = [r for r in L1 if r["mk"] in ("pra","pr","pts")]
L3 = [r for r in L2 if r["src"] in ("flip","hotover","overshoot")]
L4 = [r for r in L3 if r["notraised"]]
st(L1, "0. every OVER signal, every market   (the raw menu)")
st(L2, "1. + markets pra / pr / pts only")
st(L3, "2. + drop flip_paper, cascade, and the rest")
st(L4, "3. + THE STAR (book did not raise her 0.5+)   <- LIVE")
print("")
print("  what each step bought:")
for a, b_, lbl in ((L1,L2,"restricting markets"), (L2,L3,"dropping dead signals"), (L3,L4,"the star")):
    ra = sum((r["first"]-1) if r["won"] else -1.0 for r in a)/len(a)
    rb = sum((r["first"]-1) if r["won"] else -1.0 for r in b_)/len(b_)
    print(f"    {lbl:<26} ROI {100*ra:+6.1f}% -> {100*rb:+6.1f}%   ({100*(rb-ra):+5.1f}pp)"
          f"   kept {len(b_)}/{len(a)} bets")
print("")
print("="*112)
print("  FADE THE NON-STAR, BROKEN DOWN BY SIGNAL")
print("  (the pooled answer was 'no'. does any single signal's raised half invert?)")
print("="*112)
