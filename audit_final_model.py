...
S = [r for r in B if r["notraised"]]
OV = [r for r in S if r["src"] == "overshoot"]

print("="*104)
print("  1. OVERSHOOT STARRED, BROKEN DOWN - is it uniform, or one market carrying it?")
print("     (note: overshoot's OWN guard calls pts/pra the total-trap markets, and those are")
print("      exactly the ones our model bets. so this split matters.)")
print("="*104)
for mk in ("pra", "pr", "pts"):
    st([r for r in OV if r["mk"] == mk], f"overshoot starred  {mk}")
print("")
for mo in sorted({r["mo"] for r in OV}):
    st([r for r in OV if r["mo"] == mo], f"overshoot starred  {mo}")
print("")
print("="*104)
print("  2. HOTOVER - it is the ELSE branch in cloud_xbet.py, i.e. every over that is not")
print("     flip/newunder/usgshock/fragile/model/starout. A catch-all, not a designed signal.")
print("="*104)
HO = [r for r in B if r["src"] == "hotover"]
st(HO,                                   "hotover RAW")
st([r for r in HO if r["notraised"]],    "hotover STARRED")
st([r for r in HO if not r["notraised"]],"hotover raised")
print("")
print("  THE DECISION - model with hotover vs without:")
st([r for r in S if r["src"] in ("flip","overshoot")],           "flip + overshoot            (drop hotover)")
st([r for r in S if r["src"] in ("flip","hotover","overshoot")], "flip + hotover + overshoot  (keep)")
print("")
print("  and out of sample, split 20260718:")
dates = sorted({r["date"] for r in B}); cut = dates[int(len(dates)*0.6)]
for lbl, keys in (("drop hotover", ("flip","overshoot")), ("keep hotover", ("flip","hotover","overshoot"))):
    st([r for r in S if r["src"] in keys and r["date"] <  cut], f"  {lbl}  IN")
    st([r for r in S if r["src"] in keys and r["date"] >= cut], f"  {lbl}  OUT")
print("")
print("="*104)
print("  3. THE STAR AND THE RAISED HALF, SIDE BY SIDE, ON EVERYTHING WE BET")
print("="*104)
K = [r for r in B if r["src"] in ("flip","hotover","overshoot")]
st([r for r in K if r["notraised"]],     "STARRED  (book held or cut her number)")
st([r for r in K if not r["notraised"]], "RAISED   (book already moved it up 0.5+)")
