...
GROUPS = [("MODEL S overs", "Over", ("flip","hotover","overshoot")),
          ("newunder (ft-drought etc)", "Under", ("newunder",)),
          ("model (cold/shrink under)", "Under", ("model",)),
          ("ALL unders", "Under", ("newunder","model","starout","fragile"))]
print("="*104)
print("  THE FALSIFICATION TEST: if a RISING game total helps overs, it must HURT unders.")
print("  If both move the same way, the pattern is an artifact.")
print("="*104)
res = {}
for lbl, side, srcs in GROUPS:
    K = build(side, srcs)
    cov = sum(1 for r in K if r.get("d_tot") is not None)
    print(f"  {lbl}   {len(K)} bets, {cov} with both games priced   baseline ROI {100*roi(K):+.1f}%")
    have = [r for r in K if r.get("d_tot") is not None]
    hi = show([r for r in have if r["d_tot"] > 0], "total HIGHER than last game")
    lo = show([r for r in have if r["d_tot"] < 0], "total LOWER than last game")
    res[lbl] = (hi, lo)
    hs = show([r for r in K if r.get("d_ml") is not None and r["d_ml"] < 0],
              "more UNDERDOG than last game")
    hf = show([r for r in K if r.get("d_ml") is not None and r["d_ml"] > 0],
              "more FAVOURED than last game")
    res[lbl + "|ml"] = (hf, hs)
    print("")
print("="*104)
print("  VERDICT")
print("="*104)
o = res.get("MODEL S overs", (None, None))
for lbl in ("newunder (ft-drought etc)", "model (cold/shrink under)", "ALL unders"):
    u = res.get(lbl, (None, None))
    if None in o or None in u:
        print(f"  {lbl:<30} not enough coverage to compare"); continue
    o_gap = o[0] - o[1]        # overs: higher-total minus lower-total
    u_gap = u[0] - u[1]        # unders: same
    verdict = ("INVERTS as required" if (o_gap > 0) != (u_gap > 0)
               else "SAME DIRECTION - artifact")
    print(f"  {lbl:<30} overs gap {100*o_gap:+6.1f}pp   unders gap {100*u_gap:+6.1f}pp   {verdict}")
