...
import collections as C, itertools, random
...
for d in list(byday):
    best = {}
    for r in sorted(byday[d], key=lambda x: -x["first"]): best.setdefault(r["pl"], r)
    byday[d] = list(best.values())
allb = [r for v in byday.values() for r in v]
p_single = sum(1 for r in allb if r["won"])/len(allb)

print("="*100)
print("  MY PARLAY TEST WAS DOUBLE-COUNTING AND I NEARLY REPORTED IT")
print("="*100)
print("  I enumerated EVERY pair on each night. A 7-bet night yields 21 pairs that all share")
print("  legs, so one good night contributes 21 correlated 'wins'. That inflates both the hit")
print("  rate and the apparent correlation. Nights by size:")
sz = C.Counter(len(v) for v in byday.values())
tot_pairs = sum(n*(n-1)//2 * c for n, c in sz.items())
for n, c in sorted(sz.items()):
    if n >= 2: print(f"    {n}-bet nights: {c}  ->  {n*(n-1)//2 * c} pairs")
print(f"    total {tot_pairs} pairs from {sum(c for n,c in sz.items() if n>=2)} nights")
print("")
print("="*100)
print("  CORRECTED: NON-OVERLAPPING pairs only, each bet used once, 500 random pairings")
print("="*100)
res = []
for trial in range(500):
    n = w = 0; u = 0.0
    for d, v in byday.items():
        pool = v[:]; random.shuffle(pool)
        for i in range(0, len(pool)-1, 2):
            a, b_ = pool[i], pool[i+1]
            won = a["won"] and b_["won"]
            od = a["first"]*b_["first"]
            n += 1; w += won; u += (od-1) if won else -1.0
    if n: res.append((w/n, u/n, n))
res.sort()
mid = res[len(res)//2]
print(f"  non-overlapping 2-leg parlays: ~{mid[2]} per pass")
...
print(f"    independence would predict {100*p_single**2:.1f}%")
print("")
print("="*100)
print("  IS THERE REAL SAME-NIGHT CORRELATION? (does our signal run hot and cold together)")
print("="*100)
multi = [v for v in byday.values() if len(v) >= 2]
obs = C.Counter()
for v in multi:
    wins = sum(1 for r in v if r["won"])
    obs[f"{wins}/{len(v)}"] += 1
allornone = sum(1 for v in multi if all(r["won"] for r in v) or not any(r["won"] for r in v))
print(f"  nights with 2+ bets: {len(multi)}")
print(f"  nights where ALL won or ALL lost: {allornone} ({100*allornone/len(multi):.0f}%)")
exp_all = sum(p_single**len(v) + (1-p_single)**len(v) for v in multi)
print(f"  if legs were independent you would expect {exp_all:.1f} ({100*exp_all/len(multi):.0f}%)")
print(f"  -> {'CLUSTERED - legs are positively correlated' if allornone > exp_all*1.15 else 'roughly independent'}")
