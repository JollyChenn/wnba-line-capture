...
for r in M:
    ...
    r["p"] = io/(io+iu)
K = [r for r in M if r["mk"] in ("pra","pr","pts") and r["src"] in ("flip","hotover","overshoot")]
def roi(rows, out):
    return sum((r["oo"]-1) if out(r) else -1.0 for r in rows)/len(rows) if rows else 0.0
CFG = [
    ("MODEL S  (cut or held)      LIVE", lambda r: r["dline"] < 0.5),
    ("the +1 band alone",               lambda r: 0.5 <= r["dline"] < 2.0),
    ("LOOSENED (allow +1)",             lambda r: r["dline"] < 2.0),
    ("raised +2 or more",               lambda r: r["dline"] >= 2.0),
]
print("="*104)
print("  NULL TEST - outcomes drawn from each line's own de-vigged book price")
print("="*104)
print(f"  {'config':<34} {'n':>4} {'real ROI':>10} {'null med':>10} {'null p95':>10} {'p':>8}  95% CI")
T = 4000
for lbl, fn in CFG:
    ...
    sims = sorted(roi(rows, (lambda dd: (lambda r: dd[id(r)]))(d)) for d in
                  [{id(r): (random.random() < r["p"]) for r in rows} for _ in range(T)])
    beat = sum(1 for x in sims if x >= rl)
    w = sum(1 for r in rows if r["over_won"]); hit = w/n
    se = math.sqrt(hit*(1-hit)/n); avg = sum(r["oo"] for r in rows)/n
    print(f"  {lbl:<34} {n:>4} {100*rl:+9.1f}% {100*sims[T//2]:+9.1f}% "
          f"{100*sims[int(T*0.95)]:+9.1f}% {beat/T:8.4f}  {100*hit:.1f}% +/-{100*1.96*se:.1f}pp"
          f"  be {100/avg:.1f}%")
