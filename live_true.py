...
# EXACTLY what the card bets: pra/pr/pts x flip/hotover/overshoot x starred
K = [r for r in B if r["mk"] in ("pra","pr","pts")]
print("="*104)
print("  THE LIVE MODEL AS THE CARD ACTUALLY WIRES IT (market filter INCLUDED)")
print("="*104)
for lbl, keys in (("flip only", ("flip",)), ("flip + hotover", ("flip","hotover")),
                  ("flip + hotover + overshoot   <- LIVE", ("flip","hotover","overshoot"))):
    st([r for r in K if r["src"] in keys and r["notraised"]], lbl)
print("")
st([r for r in K if r["src"] in ("flip","hotover","overshoot") and not r["notraised"]],
   "the RAISED half (not bet)")
print("")
dates = sorted({r["date"] for r in B}); cut = dates[int(len(dates)*0.6)]
print(f"  out of sample, split {cut}:")
LIVE = [r for r in K if r["src"] in ("flip","hotover","overshoot") and r["notraised"]]
st([r for r in LIVE if r["date"] <  cut], "  LIVE  IN")
st([r for r in LIVE if r["date"] >= cut], "  LIVE  OUT")
print("")
for mo in sorted({r["mo"] for r in LIVE}):
    st([r for r in LIVE if r["mo"] == mo], f"  LIVE  {mo}")
print("")
days = len({r["date"] for r in B})
print(f"  volume: {len(LIVE)} bets over {days} match-days = {len(LIVE)/days:.2f} per night")
