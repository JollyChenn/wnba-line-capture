...
def _med(pl, mk, dt, n=10):
    v = [g for g in plog.get(pl, []) if g["date"] < dt][-n:]
    return _stats.median(g[mk] for g in v) if len(v) >= 6 else None
...
for (dt, pl, mk), v in byid.items():
    ...
    B.append(dict(date=dt, mo=dt[:6], pl=pl, mk=mk, src=src, first_ln=first_ln, close_ln=cl, med=_med(pl, mk, dt),
...
OV = [r for r in B if r["src"] == "overshoot" and r["mk"] in ("pra","pr","pts") and r["med"] is not None]
for r in OV: r["gap"] = r["med"] - r["first_ln"]      # how far BELOW her median the line sits
print(f"{len(OV)} overshoot bets in pra/pr/pts with a usable 10-game median")
print("")
print("="*114)
print("  YOUR HYPOTHESIS: for overshoot, 'raised' may be the wrong filter. What matters could be")
print("  how far below her median the line STILL sits after the book moved it.")
print("="*114)
print("  by GAP (median minus line), ignoring raised/starred entirely:")
for lo, hi, lbl in ((-99, 0, "line AT or ABOVE her median"), (0, 2, "0-2 below"),
                    (2, 4, "2-4 below"), (4, 6, "4-6 below"), (6, 99, "6+ below")):
    st([r for r in OV if lo <= r["gap"] < hi], f"    {lbl}")
print("")
print("  and now the cross: does the gap explain what 'raised' was picking up?")
for star in (True, False):
    lbl = "STARRED" if star else "RAISED "
    grp = [r for r in OV if r["notraised"] == star]
    print(f"    {lbl}  (n={len(grp)})  mean gap {sum(r['gap'] for r in grp)/len(grp):+.2f}")
    for lo, hi, sub in ((-99, 2, "gap < 2"), (2, 99, "gap >= 2")):
        st([r for r in grp if lo <= r["gap"] < hi], f"      {lbl} {sub}")
    print("")
print("="*114)
print("  HEAD TO HEAD on the same bets: which single filter is better for overshoot?")
print("="*114)
st([r for r in OV if r["notraised"]],       "STAR   (book did not raise 0.5+)")
st([r for r in OV if r["gap"] >= 2],        "GAP    (line still 2+ below her median)")
st([r for r in OV if r["notraised"] and r["gap"] >= 2], "BOTH")
st(OV,                                       "neither - every overshoot in these markets")
