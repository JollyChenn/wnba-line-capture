...
for SRC in ("flip_paper", "cascade"):
    K = [r for r in B if r["src"] == SRC and r["mk"] in ("pra","pr","pts")]
    print("="*108)
    print(f"  {SRC.upper()}  -  {len(K)} graded bets in our markets. Is there a pattern inside?")
    print("="*108)
    st(K, "  everything")
    st([r for r in K if r["star"]],     "  STARRED (the filter that works elsewhere)")
    st([r for r in K if not r["star"]], "  not starred")
    print("")
    for t in ("STRONG", "SOLID", "THIN", "CASC"):
        st([r for r in K if r["tier"] == t], f"  tier {t}")
    print("")
    ev = sorted(r["evf"] for r in K if r["evf"] is not None)
    if ev:
        m = ev[len(ev)//2]
        st([r for r in K if r["evf"] is not None and r["evf"] >= m], f"  engine EV >= median ({m:.3f})")
        st([r for r in K if r["evf"] is not None and r["evf"] <  m], f"  engine EV <  median")
    print("")
    P = [r for r in K if r["pedge"] is not None]
    print(f"  Pinnacle reference available on {len(P)} of {len(K)}")
    st([r for r in P if r["pedge"] >  0.5], "  PINNACLE line ABOVE ours by 0.5+ (cheap over)")
    st([r for r in P if abs(r["pedge"]) <= 0.5], "  Pinnacle agrees with ours")
    st([r for r in P if r["pedge"] < -0.5], "  Pinnacle line BELOW ours (our line is rich)")
    print("")
    st([r for r in P if r["pedge"] > 0.5 and r["star"]], "  Pinnacle-cheap AND starred")
    print("")
