...
print(f"{len(B)} deduped OVER bets on the BIG audit universe")
print("")

def st(rows, label, minn=20):
    n = len(rows)
    if n < minn:
        print(f"  {label:<44} n={n:<4} too few"); return None
...
print("="*104)
print("  A. THE STAR, APPLIED TO EVERY SIGNAL ON THE BIG SAMPLE")
print("="*104)
import collections as C
for s_ in sorted({r["src"] for r in B}, key=lambda x: -len([r for r in B if r["src"]==x])):
    rows = [r for r in B if r["src"] == s_]
    if len(rows) < 20: continue
    st(rows,                                        f"{s_}  RAW")
    st([r for r in rows if r["notraised"]],         f"{s_}  STARRED")
    st([r for r in rows if not r["notraised"]],     f"{s_}  raised")
    print("")

print("="*104)
print("  B. TIME SPLIT - does it survive out of sample? (this is what killed every other model)")
print("="*104)
dates = sorted({r["date"] for r in B}); cut = dates[int(len(dates)*0.6)]
print(f"  split at {cut}   (first 60% of match-days in sample, last 40% out)")
print("")
GROUPS = [("flip+hotover STARRED  <- LIVE", lambda r: r["src"] in ("flip","hotover") and r["notraised"]),
          ("overshoot STARRED",             lambda r: r["src"] == "overshoot" and r["notraised"]),
          ("overshoot RAW",                 lambda r: r["src"] == "overshoot"),
          ("flip+hotover+overshoot STARRED",lambda r: r["src"] in ("flip","hotover","overshoot") and r["notraised"])]
for label, fn in GROUPS:
    rows = [r for r in B if fn(r)]
    st([r for r in rows if r["date"] <  cut], f"{label}  IN")
    st([r for r in rows if r["date"] >= cut], f"{label}  OUT")
    print("")

print("="*104)
print("  C. MONTH BY MONTH - one hot streak, or a real thing?")
print("="*104)
for label, fn in GROUPS[:2]:
    print(f"  {label}")
    for mo in sorted({r["mo"] for r in B}):
        st([r for r in B if fn(r) and r["mo"] == mo], f"    {mo}", minn=8)
    print("")
