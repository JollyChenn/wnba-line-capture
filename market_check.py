...
print("="*96)
print("  IS pts BAD EVERYWHERE, OR ONLY INSIDE OVERSHOOT?")
print("  (cutting a market on 16 bets is curve-fitting unless it replicates elsewhere)")
print("="*96)
for src in ("flip", "overshoot", "hotover"):
    print(f"  {src} starred:")
    for mk in ("pra", "pr", "pts"):
        st([r for r in S if r["src"] == src and r["mk"] == mk], f"    {mk}")
    print("")
print("  ALL THREE SIGNALS POOLED, by market:")
for mk in ("pra", "pr", "pts"):
    st([r for r in S if r["src"] in ("flip","hotover","overshoot") and r["mk"] == mk], f"    {mk}")
print("")
print("  and the blind board baseline for each market (what a coin-flip bettor gets):")
seen = {}
for r in B:
    seen.setdefault(r["mk"], r["base"])
for mk in ("pra", "pr", "pts"):
    if mk in seen: print(f"    {mk:<6} blind Over hit rate {100*seen[mk]:.1f}%")
