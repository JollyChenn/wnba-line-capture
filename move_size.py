...
K = [r for r in B if r["mk"] in ("pra","pr","pts") and r["src"] in ("flip","hotover","overshoot")
     and r["prev_ln"] is not None]
for r in K: r["mv"] = r["first_ln"] - r["prev_ln"]        # + = book raised, - = book cut
...
print(f"{len(K)} model candidates with a previous-game line to compare")
print("")
print("="*104)
print("  DOES THE SIZE OF THE MOVE MATTER? line tonight MINUS her previous game's line")
print("="*104)
BANDS = [(-99, -2.0, "CUT 2.0+   (book slashed it)"),
         (-2.0, -1.0, "cut 1.0-2.0"),
         (-1.0, -0.001, "cut 0.5-1.0"),
         (-0.001, 0.001, "HELD exactly"),
         (0.001, 1.0,  "raised 0.5-1.0"),
         (1.0, 2.0,   "raised 1.0-2.0"),
         (2.0, 3.0,   "raised 2.0-3.0"),
         (3.0, 99,    "RAISED 3.0+   (book slammed it)")]
for lo, hi, lbl in BANDS:
    st([r for r in K if lo <= r["mv"] < hi], f"  {lbl}")
print("")
print("  grouped the way the model actually splits them:")
st([r for r in K if r["mv"] < 0.5],  "  STARRED  (cut or held)          <- WE BET")
st([r for r in K if r["mv"] >= 0.5], "  RAISED   (any amount)           <- we skip")
print("")
print("="*104)
print("  IS A CUT BETTER THAN A HOLD? the star lumps them together - should it?")
print("="*104)
st([r for r in K if r["mv"] < -0.001],                       "  CUT  (book lowered her number)")
st([r for r in K if -0.001 <= r["mv"] < 0.001],              "  HELD (exactly the same number)")
print("")
print("="*104)
print("  AND WITHIN THE RAISED GROUP - is a BIG raise different from a small one?")
print("="*104)
st([r for r in K if 0.5 <= r["mv"] < 2.0],  "  raised a little (0.5 - 2.0)")
st([r for r in K if r["mv"] >= 2.0],        "  raised a lot (2.0+)")
print("")
print("  and fading each raised band (betting the UNDER instead):")
print("  [needs both sides quoted - see the strict-universe fade test, pooled answer was -4.7%]")
