...
print("="*106)
print("  WHICH COMBINATION OF STARRED SIGNALS?  (all starred = book did not raise 0.5+)")
print("="*106)
S = [r for r in B if r["notraised"]]
for lbl, keys in [("flip only", ("flip",)),
                  ("flip + hotover            <- LIVE NOW", ("flip","hotover")),
                  ("flip + overshoot", ("flip","overshoot")),
                  ("flip + hotover + overshoot", ("flip","hotover","overshoot")),
                  ("flip + hotover + overshoot + flip_paper", ("flip","hotover","overshoot","flip_paper"))]:
    st([r for r in S if r["src"] in keys], lbl)
print("")
print("="*106)
print("  DO THEY OVERLAP? (if overshoot fires on the same nights as flip it adds nothing)")
print("="*106)
fk = {(r["date"], r["pl"], r["mk"]) for r in S if r["src"] in ("flip","hotover")}
ok_ = {(r["date"], r["pl"], r["mk"]) for r in S if r["src"] == "overshoot"}
print(f"  flip/hotover starred bets   {len(fk)}")
print(f"  overshoot starred bets      {len(ok_)}")
print(f"  the same player-market-night in both  {len(fk & ok_)}")
print("  (src is the FIRST signal logged for that bet, so these are disjoint by construction -")
print("   overshoot bets are genuinely EXTRA bets, not relabels of ones we already take)")
print("")
print("="*106)
print("  VOLUME - what this actually changes on your phone")
print("="*106)
days = len({r["date"] for r in B})
for lbl, keys in [("flip + hotover            <- LIVE NOW", ("flip","hotover")),
                  ("flip + hotover + overshoot", ("flip","hotover","overshoot"))]:
    n = len([r for r in S if r["src"] in keys])
    print(f"  {lbl:<46} {n} bets over {days} match-days = {n/days:.2f} per night")
