...
print("="*100)
print("  IS MY 'CLOSE IS THE BEST PRICE' NUMBER HONEST?")
print("  won= is settled against first_ln. close price is quoted at close_ln. If those differ,")
print("  I am crediting a different line's odds to this line's outcome - a fake edge.")
print("="*100)
same = [r for r in S if r["close_ln"] == r["first_ln"]]
diff = [r for r in S if r["close_ln"] != r["first_ln"]]
print(f"  starred bets with a close price      {len(S)}")
print(f"    close quoted at the SAME line      {len(same)}")
print(f"    close quoted at a DIFFERENT line   {len(diff)}   <- contaminated")
print("")
def st(rows, label):
    n = len(rows)
    if n < 10:
        print(f"  {label:<42} n={n} too few"); return
    w = sum(1 for r in rows if r["won"])
    for k in ("first", "close"):
        u = sum((r[k]-1) if r["won"] else -1.0 for r in rows)
        mo = sum(r[k] for r in rows)/n
        print(f"  {label:<42} {k:<6} n={n:<4} {100*w/n:5.1f}%  mean {mo:.3f}  {u:+7.2f}u  ROI {100*u/n:+6.1f}%")
    print("")
st(S,    "ALL starred (what I quoted you)")
st(same, "CLEAN - close is the same line as the bet")
st(diff, "CONTAMINATED - close is a different line")
if diff:
    up = sum(1 for r in diff if r["close_ln"] > r["first_ln"])
    print(f"  of the {len(diff)} contaminated, {up} had the line move UP by close")
    print("  (a higher line quotes LONGER odds, which would flatter the close column)")
