...
for r in M:
    ...
    r["p"] = io/(io+iu)
K = [r for r in M if r["mk"] in ("pra","pr","pts")]

CFG = [
    ("flip only, starred",            lambda r: r["src"] == "flip" and r["dline"] < 0.5),
    ("S_prev flip+hotover, starred",  lambda r: r["src"] in ("flip","hotover") and r["dline"] < 0.5),
    ("MODEL S +overshoot, starred",   lambda r: r["src"] in ("flip","hotover","overshoot") and r["dline"] < 0.5),
    ("S_nostar, no star",             lambda r: r["src"] in ("flip","hotover","overshoot")),
]
def roi(rows, out):
    return sum((r["oo"]-1) if out(r) else -1.0 for r in rows)/len(rows) if rows else 0.0

print("="*104)
print("  EACH CONFIG vs ITS OWN NULL (outcomes drawn from each line's de-vigged book price)")
print("  This asks 'is THIS rule better than no edge', pre-specified - not 'is the best of my")
print("  search significant', which is a different and much weaker claim.")
print("="*104)
print(f"  {'config':<32} {'n':>4} {'real ROI':>10} {'null med':>10} {'null p95':>10} {'p':>8}  95% CI on win rate")
TRIALS = 3000
for lbl, fn in CFG:
    rows = [r for r in K if fn(r)]
    n = len(rows)
    if n < 10: continue
    rl = roi(rows, lambda r: r["over_won"])
    sims = sorted(roi(rows, (lambda dd: (lambda r: dd[id(r)]))(d)) for d in
                  [{id(r): (random.random() < r["p"]) for r in rows} for _ in range(TRIALS)])
    beat = sum(1 for x in sims if x >= rl)
    w = sum(1 for r in rows if r["over_won"]); hit = w/n
    se = math.sqrt(hit*(1-hit)/n)
    print(f"  {lbl:<32} {n:>4} {100*rl:+9.1f}% {100*sims[TRIALS//2]:+9.1f}% "
          f"{100*sims[int(TRIALS*0.95)]:+9.1f}% {beat/TRIALS:8.4f}  "
          f"{100*hit:.1f}% +/- {100*1.96*se:.1f}pp  ({100*(hit-1.96*se):.1f} to {100*(hit+1.96*se):.1f})")
print("")
print("="*104)
print("  BREAK-EVEN CHECK: how far can the true win rate fall before each rule stops paying?")
print("="*104)
for lbl, fn in CFG:
    rows = [r for r in K if fn(r)]
    n = len(rows)
    if n < 10: continue
    w = sum(1 for r in rows if r["over_won"]); hit = w/n
    avg = sum(r["oo"] for r in rows)/n
    be = 1/avg
    se = math.sqrt(hit*(1-hit)/n)
    lo = hit - 1.96*se
    print(f"  {lbl:<32} hits {100*hit:5.1f}%  needs {100*be:5.1f}%  cushion {100*(hit-be):+5.1f}pp"
          f"   worst case in CI {100*lo:5.1f}%  ->  {'STILL PAYS' if lo > be else 'could be a loser'}")
