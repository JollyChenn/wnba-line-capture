...
R = [r for r in B if not r["notraised"] and r["src"] in ("flip","hotover","overshoot")]
n = len(R); w = sum(1 for r in R if r["won"])
u = sum((r["first"]-1) if r["won"] else -1.0 for r in R)
b = sum(r["base"] for r in R)/n
print(f"RAISED (unstarred) flip+hotover+overshoot: n={n} {100*w/n:.1f}% {u:+.2f}u ROI {100*u/n:+.1f}% alpha {100*(w/n-b):+.1f}pp")
