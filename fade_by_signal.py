...
def fade(rows, label, minn=10):
    ...
    ub = sum(r["ubase"] for r in rows)/n
    avg = sum(r["uo"] for r in rows)/n
    be  = 100.0/avg
    print(f"  {label:<40} n={n:<4} under hits {100*uw/n:5.1f}%  avg {avg:.3f}"
          f"  break-even {be:5.1f}%  {uu:+7.2f}u  ROI {100*uu/n:+6.1f}%  alpha {100*(uw/n-ub):+5.1f}pp")
print("="*112)
print("  FADE THE NON-STAR (raised), BY SIGNAL. Betting the UNDER at the under's own price.")
print("="*112)
R = [r for r in K if r["dline"] >= 0.5]
fade(R, "ALL raised, pooled")
print("")
for s_ in ("flip", "hotover", "overshoot"):
    fade([r for r in R if r["src"] == s_], f"  raised {s_}")
print("")
print("  for reference, fading the STARRED ones (what we bet):")
fade([r for r in K if r["dline"] < 0.5], "  starred, faded")
print("")
print("="*112)
print("  AND THE LIVE MODEL, WITH THE MARKET FILTER APPLIED (pra/pr/pts)")
print("="*112)
L = [r for r in K if r["src"] in ("flip","hotover","overshoot") and r["dline"] < 0.5]
n = len(L); w = sum(1 for r in L if r["over_won"])
u = sum((r["oo"]-1) if r["over_won"] else -1.0 for r in L)
b = sum(r["obase"] for r in L)/n
print(f"  n={n}  {100*w/n:.1f}%  {u:+.2f}u  ROI {100*u/n:+.1f}%  alpha {100*(w/n-b):+.1f}pp")
